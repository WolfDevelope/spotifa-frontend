...
const RisingAlternativeIndieArtists = () => {
  ...
  // Fetch rising alternative indie artists from API
  useEffect(() => {
    const fetchRisingArtists = async () => {
      try {
        setLoading(true);
        // Sử dụng getAllArtists và lấy một phần để tạo "rising alternative indie" effect
        const response = await musicService.getAllArtists();
        if (response.success) {
          // Lấy artists từ vị trí 10-20 và shuffle để tạo hiệu ứng "alternative"
          const risingArtists = response.data.slice(10, 20).sort(() => Math.random() - 0.5);
          setArtists(risingArtists);
        } else {
          setError('Failed to load rising alternative indie artists');
        }
      } catch (err) {
        setError('Failed to load rising alternative indie artists');
        console.error('Error fetching rising alternative indie artists:', err);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <section className="mb-8" id="risingAlternativeIndieArtistsList">
        <h2 className="text-2xl font-bold mb-3">
          Rising <span className="text-pink-400">Alternative & Indie Artists</span>
        </h2>
  ...
  if (!artists.length) {
    return (
      <section className="mb-8" id="risingAlternativeIndieArtistsList">
        ...
        <div className="text-center py-8">
          <p className="text-gray-400 mb-4">No rising alternative indie artists available</p>
        </div>
      </section>
    );
  }
