...
import AlbumCard from './AlbumCard_API'; // Sử dụng AlbumCard API version
...
const NewAlbums = () => {
  ...
  // Fetch new albums from API
  useEffect(() => {
    const fetchNewAlbums = async () => {
      try {
        setLoading(true);
        // Sử dụng getAllAlbums thay vì getNewAlbums vì backend chưa có dữ liệu
        const response = await musicService.getAllAlbums();
        console.log('NewAlbums API response:', response);
        if (response.success) {
          // Lấy albums cuối cùng làm "new albums" (reverse order)
          const newAlbums = response.data.slice().reverse().slice(0, 10);
          console.log('NewAlbums data:', newAlbums);
          setAlbums(newAlbums);
        } else {
          console.log('NewAlbums error:', response);
  ...
  if (!albums.length) {
    return (
      <section className="mb-8" id="newAlbumsList">
        ...
        <div className="text-center py-8">
          <p className="text-gray-400 mb-4">No new albums available</p>
        </div>
      </section>
    );
  }
