// src/components/TopAlbums_API.jsx
import React, { useState, useEffect } from 'react';
import AlbumCard from './AlbumCard_API'; // Import AlbumCard API version
...
const TopAlbums = () => {
  ...
  useEffect(() => {
    const fetchTopAlbums = async () => {
      try {
        setLoading(true);
        // Sử dụng getAllAlbums thay vì getTopAlbums vì backend chưa có dữ liệu
        const response = await musicService.getAllAlbums();
        console.log('TopAlbums API response:', response);
        if (response.success) {
          // Lấy albums đầu tiên làm "top albums"
          const topAlbums = response.data.slice(0, 10);
          console.log('TopAlbums data:', topAlbums);
          setAlbums(topAlbums);
        } else {
          console.log('TopAlbums error:', response);
      ...
      <section className="mb-8">
        ...
        <div className="flex flex-wrap gap-2">
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 flex-grow">
            {[...Array(5)].map((_, index) => (
              <div key={index} className="w-1/2 sm:w-1/3 md:w-1/4 lg:w-1/5 xl:w-1/6 px-2 mb-4">
  ...
  if (!albums || albums.length === 0) {
    return (
      <section className="mb-8">
        ...
        <div className="text-center py-8">
          <p className="text-gray-400 mb-4">No top albums available</p>
        </div>
      </section>
    );
  }
  ...
  return (
    <section className="mb-8">
      ...
      {albumsToShow.map(album => (
          <div key={album._id} className="w-1/2 sm:w-1/3 md:w-1/4 lg:w-1/5 xl:w-1/6 px-2 mb-4">
