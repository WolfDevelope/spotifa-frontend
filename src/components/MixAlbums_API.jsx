...
const MixAlbums = () => {
  ...
  // Fetch mix albums from API
  useEffect(() => {
    const fetchMixAlbums = async () => {
      try {
        setLoading(true);
        ...
        setError('Failed to load mix albums');
        console.error('Error fetching mix albums:', err);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <section className="mb-8" id="mixAlbumsList">
        <h2 className="text-2xl font-bold mb-3">
          Mix <span className="text-pink-400">Albums</span>
        </h2>
