// src/components/WeeklyTopSongs_API.jsx
import React, { useState, useEffect } from 'react';
import SongCard from './SongCard_API'; // Import SongCard API version
...
const WeeklyTopSongs = () => {
  ...
  // Fetch weekly top songs from API
  useEffect(() => {
    const fetchWeeklyTopSongs = async () => {
      try {
        setLoading(true);
        const response = await musicService.getWeeklyTopSongs();
        if (response.success) {
          setWeeklySongs(response.data);
        } else {
          setError('Failed to load weekly top songs');
        }
      } catch (err) {
        setError('Failed to load weekly top songs');
        console.error('Error fetching weekly top songs:', err);
      } finally {
        setLoading(false);
      }
    };
