import React, { useState, useEffect } from 'react';
import ArtistsCard from './ArtistsCard_API';
import musicService from '../services/musicService';

const RecentlyViralArtists = () => {
  const [artists, setArtists] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [showAll, setShowAll] = useState(false);

  // Fetch recently viral artists from API
  useEffect(() => {
    const fetchViralArtists = async () => {
      try {
        setLoading(true);
        // Sử dụng getAllArtists và lấy artists cuối cùng để tạo "recently viral" effect
        const response = await musicService.getAllArtists();
        if (response.success) {
          // Lấy artists cuối cùng (reverse) để tạo hiệu ứng "recently viral"
          const viralArtists = response.data.slice().reverse().slice(0, 10);
          setArtists(viralArtists);
        } else {
          setError('Failed to load recently viral artists');
        }
      } catch (err) {
        setError('Failed to load recently viral artists');
        console.error('Error fetching recently viral artists:', err);
      } finally {
        setLoading(false);
      }
    };

    fetchViralArtists();
  }, []);

  if (loading) {
    return (
      <section className="mb-8" id="recentlyViralArtistsList">
        <h2 className="text-2xl font-bold mb-3">
          Recently Viral <span className="text-pink-400">Artists</span>
        </h2>
        <div className="flex flex-wrap gap-2">
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 flex-grow">
            {[...Array(5)].map((_, index) => (
              <div key={index} className="text-center">
                <div className="w-32 h-32 bg-gray-600 rounded-full mx-auto mb-2 animate-pulse"></div>
                <div className="h-4 bg-gray-600 rounded mb-1 animate-pulse"></div>
              </div>
            ))}
          </div>
        </div>
      </section>
    );
  }

  if (error) {
    return (
      <section className="mb-8" id="recentlyViralArtistsList">
        <h2 className="text-2xl font-bold mb-3">
          Recently Viral <span className="text-pink-400">Artists</span>
        </h2>
        <div className="text-center py-8">
          <p className="text-red-400 mb-4">{error}</p>
          <button 
            onClick={() => window.location.reload()} 
            className="px-4 py-2 bg-purple-500 hover:bg-purple-600 rounded-lg transition"
          >
            Retry
          </button>
        </div>
      </section>
    );
  }

  // Lấy danh sách artist để hiển thị
  const artistsToShow = showAll ? artists : artists.slice(0, 5);

  if (!artists.length) {
    return (
      <section className="mb-8" id="recentlyViralArtistsList">
        <h2 className="text-2xl font-bold mb-3">
          Recently Viral <span className="text-pink-400">Artists</span>
        </h2>
        <div className="text-center py-8">
          <p className="text-gray-400 mb-4">No recently viral artists available</p>
        </div>
      </section>
    );
  }

  return (
    <section className="mb-8" id="recentlyViralArtistsList">
      <h2 className="text-2xl font-bold mb-3">
        Recently Viral <span className="text-pink-400">Artists</span>
      </h2>
      <div className="flex flex-wrap gap-2">
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 flex-grow">
          {artistsToShow.map(artist => (
            <ArtistsCard key={artist._id} artist={artist} />
          ))}
        </div>
        {/* View All Button */}
        <div className="flex flex-col items-center justify-center flex-shrink-0">
          <button
            className="cursor-pointer w-14 h-14 bg-[#232323] rounded-full flex items-center justify-center shadow-lg mb-2 hover:bg-pink-500 transition"
            onClick={() => setShowAll((prev) => !prev)}
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="w-8 h-8 text-white"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
              <circle cx="12" cy="12" r="10" stroke="white" strokeWidth="2" fill="none" />
              <path
                stroke="white"
                strokeWidth="2"
                strokeLinecap="round"
                strokeLinejoin="round"
                d="M12 8v8m-4-4h8"
              />
            </svg>
          </button>
          <span className="text-white font-semibold text-base">
            {showAll ? 'Collapse' : 'View All'}
          </span>
        </div>
      </div>
    </section>
  );
};

export default RecentlyViralArtists;
